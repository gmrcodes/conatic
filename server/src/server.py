...
# Lock para garantizar thread-safety en operaciones SQLite
...
class ServidorGridTerminales:
    def __init__(self, root):
        self.root = root
        self.root.title(f"Panel de Control de Terminales - Servidor [{SISTEMA_OPERATIVO}]")
    ...
    def configurar_estilos(self):
        ...
        style.configure("Treeview", background=COLOR_PANEL, foreground=COLOR_TEXT_MAIN,
                        fieldbackground=COLOR_PANEL, rowheight=30, borderwidth=0, font=(FUENTE_BASE, 10))
        style.configure("Treeview.Heading", background="#2d2d2d", foreground=COLOR_TEXT_MAIN,
                        relief="flat", font=(FUENTE_BASE, 10, "bold"))
    ...
    def crear_interfaz_grid(self):
        ...
        tk.Label(panel_control, text="DESBLOQUEO REMOTO", font=(FUENTE_BASE, 12, "bold"), fg=COLOR_ACCENT, bg=COLOR_PANEL).pack(pady=15, padx=15, anchor="w")
        ...
        chk_offline = tk.Checkbutton(
            ...
            activeforeground=COLOR_TEXT_MAIN, font=(FUENTE_BASE, 10, "bold"), cursor="hand2"
        )
        ...
        self.lbl_pc_seleccionada = tk.Label(panel_control, text="Ninguna (Seleccione en la tabla)", font=(FUENTE_BASE, 11, "bold"), fg=COLOR_TEXT_MAIN, bg=COLOR_PANEL)
        ...
        self.entry_id = tk.Entry(frame_busqueda, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, insertbackground='white', relief="solid", bd=1, font=(FUENTE_BASE, 11))
        self.entry_id.pack(side="left", expand=True, fill="x", pady=2)
        self.entry_id.bind("<Return>", lambda e: self.buscar_usuario_por_id())

        btn_buscar = tk.Button(frame_busqueda, text="🔍", command=self.buscar_usuario_por_id, bg="#3e3e42", fg=COLOR_TEXT_MAIN, bd=0, font=(FUENTE_BASE, 9, "bold"), width=4, cursor="hand2")
        ...
        self.entry_nombre = tk.Entry(panel_control, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, insertbackground='white', relief="solid", bd=1, font=(FUENTE_BASE, 11))
        ...
        self.entry_tiempo = tk.Entry(panel_control, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, insertbackground='white', relief="solid", bd=1, font=(FUENTE_BASE, 11))
        ...
        self.btn_activar = tk.Button(panel_control, text="🚀 Desbloquear Terminal", command=self.enviar_desbloqueo_a_cliente,
                                bg=COLOR_SUCCESS, fg=COLOR_TEXT_MAIN, bd=0, relief="flat", font=(FUENTE_BASE, 11, "bold"), cursor="hand2")
        ...
        tk.Label(area_grid, text="MONITOREO DE SALA EN TIEMPO REAL", font=(FUENTE_BASE, 12, "bold"), fg=COLOR_TEXT_MAIN, bg=COLOR_BG).pack(pady=10, anchor="w")
    ...
    def mostrar_menu_contextual(self, event):
        ...
        if item:
            ...
            if "Activo" in estado_pc:
                menu.add_command(label="⏸️ Pausar Terminal", command=lambda: self.enviar_control_accion(id_pc, "pausar_terminal"))
                menu.add_command(label="🛑 Terminar Sesión", command=lambda: self.enviar_control_accion(id_pc, "bloquear_terminal"))
                menu.add_command(label="🔄 Trasladar Usuario", command=lambda: self.ventana_trasladar_usuario(id_pc))
                menu.add_separator()
            elif "Pausado" in estado_pc:
                menu.add_command(label="▶️ Reanudar Sesión", command=lambda: self.enviar_control_accion(id_pc, "reanudar_terminal"))
                menu.add_command(label="🛑 Terminar Sesión", command=lambda: self.enviar_control_accion(id_pc, "bloquear_terminal"))
                menu.add_separator()

            menu.add_command(label="💬 Enviar Mensaje", command=lambda: self.ventana_escribir_mensaje(id_pc))
    ...
    def ventana_escribir_mensaje(self, id_pc):
        ...
        tk.Label(v_msg, text=f"Escribe el mensaje para la PC {id_pc}:", fg=COLOR_TEXT_MAIN, bg=COLOR_PANEL, font=(FUENTE_BASE, 10, "bold")).pack(pady=10)
        txt_mensaje = tk.Entry(v_msg, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, insertbackground="white", relief="solid", bd=1, font=(FUENTE_BASE, 11), width=35)
        ...
        tk.Button(v_msg, text="✉️ Enviar Mensaje", command=despachar, bg=COLOR_ACCENT, fg=COLOR_TEXT_MAIN, bd=0, font=(FUENTE_BASE, 10, "bold"), cursor="hand2", padx=10, pady=5).pack(pady=15)
    ...
    def ventana_trasladar_usuario(self, id_pc_origen):
        ...
        tk.Label(v_traslado, text=f"Cambiar usuario de PC N° {id_pc_origen} a:", fg=COLOR_TEXT_MAIN, bg=COLOR_PANEL, font=(FUENTE_BASE, 10, "bold")).pack(pady=10)
        entry_destino = tk.Entry(v_traslado, bg=COLOR_BG, fg=COLOR_TEXT_MAIN, insertbackground="white", relief="solid", bd=1, font=(FUENTE_BASE, 11), width=15, justify="center")
